	...
	private void deleteEntity() throws IOException
			{
				...
				response.sendRedirect("/PARK1.1/RoleServlet?type=4");//删除成功后跳转至管理页面
			}
	...
	private void updateEntity() throws UnsupportedEncodingException
			{
				...
				{
					try {
						response.sendRedirect("/PARK1.1/RoleServlet?type=4");//request是获取用户的请求，response处理用户请求。sendRedirect()函数的作用是重定向,成功更新数据后跳转至RoleMsg.jsp页面
					} catch (IOException e) {
	...
	private void insertEntity() throws UnsupportedEncodingException, IOException
			{
				...
				if(!role.checkExist(role_id))
				{
					...
					{
						out.write("<script>alert('数据添加成功！'); location.href = '/PARK1.1/RoleServlet?type=4';</script>");
					}
					else {
						out.write("<script>alert('数据添失败！'); location.href = '/PARK1.1/RoleServlet?type=4';</script>");
					}
				}
				else {
					out.write("<script>alert('主键重复，数据添加失败！'); location.href = '/PARK1.1/RoleServlet?type=4';</script>");
				}
			}
	...
	private void getEntity() throws ServletException, IOException
			{
				...
				request.getRequestDispatcher("/Control/RoleMsg.jsp").forward(request, response);//请求转发
			}
	...
	private void getEntityByWhere() throws ServletException, IOException
			{
				...
				request.getRequestDispatcher("/Control/RoleMsg.jsp").forward(request, response);
			}
	}
