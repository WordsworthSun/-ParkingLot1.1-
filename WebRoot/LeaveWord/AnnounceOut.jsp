...
<script>
	$(function()
	{
		$(".btnGo").click(function()
		{
			if($("[name=htype]").val()=="4")
			{
				location.href="<%=path %>/MessageServlet?type=4&page="+($(".txtPage").val()==""?1:$(".txtPage").val());
			}
			else
			{
				location.href="<%=path %>/MessageServlet?type=5&page="+($(".txtPage").val()==""?1:$(".txtPage").val())+"&condition="+$("[name=hcondition]").val()+"&value="+$("[name=hvalue]").val();
			}		
		});
	});
</script>
...
<div class="div_container">
				...
				out.write("<input type='hidden' name='hvalue' value='"+(request.getAttribute("value")!=null?request.getAttribute("value").toString():"")+"' />");
                		if(list!=null) //循环数据列表，生成表格行
                		{
                			for(int i=0;i<list.size();i++)
	                		{
	               				...
	               				out.print("<tr ><td align='center'>"+obj[0]+"</td><td align='center'>"+obj[1]+"</td><td align='center'>"+obj[2]+"</td><td align='center'>"+obj[3]+"</td><td align='center'>"+obj[4]+"</td><td align='center'><a href='"+path+"/LeaveWord/AnnounceEdit.jsp?message_id="+obj[4]+"' class='a_edit'>编辑</a><a href='"+path+"/MessageServlet?type=1&message_id="+obj[4]+"' class='a_del' onclick='return confirm(\"是否确认删除？\")'>删除</a></td></tr>");
	                		}
                		}
	                %>
		</tbody>
	...
	<span class="out">
	<form action="<%=path %>/MessageServlet?type=3" method="post">
        <label for="conteny">请输入公告信息：</label>
        <input type="text"  name="context" requird/>    
