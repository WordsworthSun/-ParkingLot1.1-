...
<script>
	$(function()
	{
		$(".btnGo").click(function()
		{
			if($("[name=htype]").val()=="4")
			{
				location.href="<%=path %>/CardServlet?type=4&page="+($(".txtPage").val()==""?1:$(".txtPage").val());
			}
			else
			{
				location.href="<%=path %>/CardServlet?type=5&page="+($(".txtPage").val()==""?1:$(".txtPage").val())+"&condition="+$("[name=hcondition]").val()+"&value="+$("[name=hvalue]").val();
			}		
		});
	});
</script>
...
<div class="div_container">
	<div class="searchDiv">
		<form action="<%=path %>/CardServlet" method="get">
			<span>查询条件：</span>
			<select name="condition">
			 	<option value='card_id'>会员卡编号</option>
				<option value='seat_num'>车位编号</option>
				<option value='user_name'>用户编号</option>
				<option value='user_phone'>联系方式</option>
				...
				out.write("<input type='hidden' name='hvalue' value='"+(request.getAttribute("value")!=null?request.getAttribute("value").toString():"")+"' />");
                		if(list!=null) //循环数据列表，生成表格行
                		{
                			for(int i=0;i<list.size();i++)
	                		{
	               				...
	               				out.print("<tr><td align='center'>"+obj[0]+"</td><td align='center'>"+obj[1]+"</td><td align='center'>"+obj[2]+"</td><td align='center'>"+obj[3]+"</td><td align='center'>"+obj[4]+"</td><td align='center'>"+obj[5]+"</td><td align='center'><a href='"+path+"/Control/CardEdit.jsp?card_id="+obj[0]+"' class='a_edit'>编辑</a><a href='"+path+"/CardServlet?type=1&card_id="+obj[0]+"&seat_id="+obj[1]+"' class='a_del' onclick='return confirm(\"是否确认删除？\")'>删除</a></td></tr>");
	                		}
                		}
